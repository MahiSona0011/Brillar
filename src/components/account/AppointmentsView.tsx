...
const APPOINTMENT_TYPES = [
  {
    id: "IN_STORE" as BookableAppointmentType,
    icon: MapPin,
  ...
  {
    id: "VIRTUAL" as BookableAppointmentType,
    icon: Video,
  ...
  {
    id: "BESPOKE" as BookableAppointmentType,
    icon: Calendar,
...
const TYPE_LABELS: Record<string, string> = {
  IN_STORE: "In-store consultation",
  VIRTUAL: "Virtual consultation",
  BESPOKE: "Bespoke design session",
};
...
export default function AppointmentsView({ upcoming }: { upcoming: UpcomingAppointment | null }) {
  const [type, setType] = useState<BookableAppointmentType | null>(null);
  ...
  async function book(e: React.FormEvent) {
    ...
    setError("");
    try {
      await bookAppointment({ type, date, time, notes: notes || undefined });
      setLoading(false);
      setBooked(true);
    } catch (err) {
      setLoading(false);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          ...
          {upcoming ? (
            <div className="bg-[#111111] border border-[#D4AF37]/20 rounded-[8px] p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <p className="font-cinzel text-sm tracking-[0.06em] text-[#F9F9F9]">{TYPE_LABELS[upcoming.type] ?? upcoming.type}</p>
                  <p className="font-inter text-[11px] tracking-[0.04em] text-[#888888] mt-0.5">
                    {new Date(upcoming.date).toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" })} at{" "}
                    {new Date(upcoming.date).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
                  </p>
                </div>
            ...
            <div className="bg-[#111111] border border-[#1E1E1E] rounded-[8px] p-5 text-center">
              <p className="font-inter text-xs text-[#555555]">No upcoming appointments.</p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {booked ? (
            ...
            >
              ...
              <button
                onClick={() => { setBooked(false); setType(null); setDate(""); setTime(""); setNotes(""); }}
